  ...
  (function recursive(node) {
    ...
    // Cleanup elements
      ...
      // If this is an style element with only a text node as a child
               ...
               domHelpers.HEADERS.hasOwnProperty(node.tagname) === false &&
...
Subtree.prototype.print = function () {
  ...
  (function recursive(indent, node) {
    ...
    if (node.type === 'element') {
      printout += '<' + node.tagname + ' ' + attrStringify(node.attr) + '>';
    } else {
      printout += '# ' + node.text.slice(0, 50).trim() + (node.text.trim().length > 50 ? '...' : '');
    }
    printout += '\n' + (blocky ? '\x1B[39m' : '');
