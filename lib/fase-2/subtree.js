...
Subtree.prototype.print = function () {
  ...
  (function recursive(indent, node) {
    var blocky = (node.density !== 0 && (node.blocky === false || (node.blocky === true && node.blockyChildren === false)));
    if (node.type === 'element') {
      printout += indent + '<' + (blocky ? '*' : '') + node.tagname + ' ' + attrStringify(node.attr) + '>\n';
    } else {
      printout += indent + '#' + (blocky ? '*' : '') + ' ' + node.text.slice(0, 50).trim() + (node.text.trim().length > 50 ? '...' : '') + '\n';
    }
