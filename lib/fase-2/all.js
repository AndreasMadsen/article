...
Fase2Algortime.prototype._reduceText = function () {
  ...
  // Reduce the tree using hardcoded detection
  hardcode.reduce();
  tree.reduceTree();

  // Reduce the subtree the subcontainers
  var lastcontainersize;
  var nodelist = tree.containerNodes();
  while (true) {
    analyser.collect(nodelist);
    console.log(analyser.matrix.print());

    analyser.reduce();
    tree.reduceTree();

    lastcontainersize = nodelist.length;
    nodelist = tree.containerNodes();
    if (lastcontainersize === nodelist.length) {
      break;
    }
  }
