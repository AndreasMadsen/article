...
Fase2Algortime.prototype.combine = function () {
  ...
  for (var j = 0, t = this._pairs.length; j < t; j++) {
    this._pairs[j].likelihood = mathHelpers.distance([
      this._pairs[j].title.likelihood,
      this._pairs[j].text.likelihood,
      1 - mathHelpers.adjusedValue(this._pairs[j].distance, this._adjustDistance)
    ]);
  }
  ...
  /*
  console.log(require('util').inspect(this._pairs.slice(0, 15), {
    colors: true,
    depth: 2
  }));
  */
