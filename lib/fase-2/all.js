...
function Fase2Algortime(main) {
  ...
  this._reducedTitle = null;
  this._reducedText = null;
  this._reducedImage = null;
...
Fase2Algortime.prototype._reduceTitle = function () {
  this._reducedTitle = this._pairs[0].title.node;
};

Fase2Algortime.prototype._reduceText = function () {
  this._reducedText = this._pairs[0].text.node;

  var tree = new Subtree(this._reduceText);
  
  tree.reduceTree();

  var containers = tree.containerNodes();
...
Fase2Algortime.prototype.result = function () {
  return {
    title: this._reducedTitle.getText().trim(),
    text: this._reducedText.getText().trim(),
    image: this._reducedImage ? this._reducedImage.href : null
  };
};
