...
Commonality.prototype.tagnameReducer = function () {
  ...
  for (var n = 0, r = names.length; n < r; n++) {
    ...
    console.log('testing ' + maxtagname + ' against ' + names[n], ttest(maxdataset, this.node.tagname[names[n]], ttest_options).pValue());
    if (ttest(maxdataset, this.node.tagname[names[n]], ttest_options).valid() === false) {
