...
function Article(source, callback) {
  ...
  this._parser = new Parser({
    ...
    ontext: function (text) {
      text = helpers.normalizeString(text).trim();
      if (text !== '') {
...
var metaHeadling = helpers.buildAttributeMatcher({
  property: ['rnews:headline', 'og:title', 'twitter:title'],
  name: ['title', 'hdl', 'hdl_p'],
  itemprop: ['alternativeHeadline', 'headline']
});

var metaDescription = helpers.buildAttributeMatcher({
...
Article.prototype._element = function (node) {
  ...
  if (node.tagname === 'title') {
    this._extendHistroy();
    this._titles.push({
      'text': helpers.getRawText(node).trim(),
      'node': node
    });
  }
  else if (node.tagname === 'meta' && metaHeadling(attr)) {
    this._extendHistroy();
    this._titles.push({
      'text': helpers.normalizeString(attr.content).trim(),
      'node': node
    });
  }

  // Find descriptions where position isn't helpfull
  else if (node.tagname === 'meta' && metaDescription(attr)) {
  ...
  // Find images
    ...
    this._images.push({
      'src': helpers.normalizeString(attr.src),
      'node': node
    });
  }
};
...
Article.prototype._calculateHeader = function (node) {
  var text = helpers.getRawText(node).trim();
  var distance = {
    levenshtein: [],
    worddiff: []
  };
...
function adjustedLikelihood(values, history) {
  ...
  for (var i = 0; i < l; i++) {
    adjusted += zeroDivide(values[i] - history[i][0], history[i][1] - history[i][0]);
  }
  return adjusted / l;
}
...
Article.prototype._calculateHeaderLikelihood = function (header) {
  ...
  var level = 1 - zeroDivide(header.level - history.level[0], history.level[1] - history.level[0]);
