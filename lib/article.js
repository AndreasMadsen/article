...
Article.prototype._calculateHeader = function (node) {
  ...
  for (var i = 0, l = this._titles.length; i < l; i++) {
    distance.levenshtein += levenshtein(text, this._titles[i].text);
    distance.words += worddiff(text, this._titles[i].text);
  }
...
Article.prototype._end = function () {
  ...
  var articleHeader = this._headers.sort(function (a, b) {
    // Sort by levenshtein
    var order = a.distance.words - b.distance.words;

    // then sort by worddiff
    if (order === 0) {
      order = a.distance.levenshtein - b.distance.levenshtein;
    }

    // then sort by heading level
    if (order === 0) {
