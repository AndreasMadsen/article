...
function ImageAlgoritme(main) {
  ...
  this._adjustGoldenRatio = new mathHelpers.RangeObject();
  this._adjustMetaCount = new mathHelpers.RangeObject();
  this._adjustBodyCount = new mathHelpers.RangeObject();
  this._adjustBadKeyword = new mathHelpers.RangeObject();
...
ImageAlgoritme.prototype._sizeFromUrl = function (href, parsed) {
  ...
  if (match) return new mathHelpers.ImageSize(match[1], null);
...
var USELESS_IMAGE = /icon|logo|avatar/;
...
ImageAlgoritme.prototype.end = function () {
  ...
  for (var i = 0, l = this._allKeys.length; i < l; i++) {
    ...
    if (imageObject.extname === '.gif' ||
       ...
       USELESS_IMAGE.test(imageObject.href)) {
      continue;
    }

    // Caculate scores
    imageObject.scores.metacount = imageObject.types['meta:twitter'] + imageObject.types['meta:og'];
    imageObject.scores.bodycount = bodyCountScore(
      imageObject.types.img + imageObject.types.media
    );
    ...
    this._adjustGoldenRatio.update(imageObject.size.golden);
  ...
  for (var c = 0, t = this._images.length; c < t; c++) {
    ...
    if (imageObject.size.area !== null) {
      ...
      areasize = (imageObject.types['meta:og'] > 0) ? 0.9 : 0.5;
    }

    var goldenratio = mathHelpers.adjusedValue(
      imageObject.size.golden, this._adjustGoldenRatio
    );
    ...
    imageObject.likelihood = (areasize + bodycount + metacount) / 3;
