...
var MIN_SIZE = 150*150;
ImageAlgoritme.prototype.end = function () {
  for (var i = 0, l = this._allKeys.length; i < l; i++) {
    var keyname = this._allKeys[i];

    // Ignore .gif images and images smaller than 150*150
    if (this._images[keyname].extname === '.gif' ||
       (this._images.size && this._images.size < MIN_SIZE)) {
      delete this._images[keyname];
      continue;
    }
  }
