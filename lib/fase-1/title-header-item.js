...
TitleHeaderItem.prototype.calculateLikelihood = function () {
  ...
  for (var i = 0; i < total; i++) {
    levenshteinTotal += meta[i]._levenshtein.appearAdjust(this._levenshtein, meta[i].appear);
    worddiffTotal += meta[i]._worddiff.appearAdjust(this._worddiff, meta[i].appear);
    fussyTotal += meta[i]._fussy.appearAdjust(this._fussy, meta[i].appear);
  }
  ...
  var levenshtein = levenshteinTotal / total,
      worddiff = worddiffTotal / total,
      fussy = fussyTotal / total,

  // These where created using mathematical functions there followed the
  // 1 ~ likely rule.
      wordscore = algortime._adjustLevel.adjust(this._wordscore),
