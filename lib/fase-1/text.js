...
function sentensesCount(parts) {
  // Count the amount of sentenses in a row there do not have any
  // bad paragraph in between.
  var sentensesCollection = [0];
  var sentensesInRow = 0;

  for (var i = 0, l = parts.length; i < l; i++) {
    if (CONTAIN_SENTENCE.test(parts[i]) === false) {
      if (sentensesInRow !== 0) {
        sentensesCollection.push(sentensesInRow);
        sentensesInRow = 0;
      }
      continue;
    }

    sentensesInRow += parts[i].length;
  }
  if (sentensesInRow !== 0) sentensesCollection.push(sentensesInRow);

  var maxes = [0, 0];
  for (var a = 0, r = sentensesCollection.length; a < r; a++) {
    if (sentensesCollection[a] >= maxes[1]) {
      maxes[0] = maxes[1];
      maxes[1] = sentensesCollection[a];
    }
  }
...
TextAlgoritme.prototype.end = function () {
  ...
  for (var i = 0; i < l; i++) {
    for (var j = i; j < l; j++) {
      ...
      if (commonParentMap.hasOwnProperty(parentNode.identifyer) === false) {
        var parentText = parentNode.getText().trim();

        // The size is crazy big, even if it is correct it will take a long
        // time to process
        if (parentText.length > 20000) continue;

        // If another node contains the same text, don't create a new parent
        // just increase its counter
        var identifyer = textMap[parentText];
        if (identifyer !== undefined) {
          count = commonParentMap[identifyer].count += 1;
          this._adjustContainerCount.update(count);
          continue;
        }

        // Map the text to the commonParentMap identifyer
        textMap[parentText] = parentNode.identifyer;

        var parent = commonParentMap[parentNode.identifyer] = {
          'node': parentNode,
          'count': 0,
          'shortlines': 0,
          'linebreaks': 0,
        ...
        // Count the content of the 2 bigest groupes of sentenses
        var sentenses = sentensesCount(linearray);
        parent.sentenses = sentenses;
        this._adjustSentensesCount.update(parent.sentenses);

        // Update paragraph count
        parent.paragraphs = parentNode.countTagname('p');
        this._adjustParagraphCount.update(parent.paragraphs);

        // Add the object key so we don't have to do a Object.keys later
        keys.push(parentNode.identifyer);
      ...
      count = commonParentMap[parentNode.identifyer].count += 1;
      this._adjustContainerCount.update(count);
    }
  }
  ...
  for (var n = 0, t = keys.length; n < t; n++) {
    ...
    var adjusedSentenseCount = mathHelpers.adjusedValue(item.sentenses, this._adjustSentensesCount);
    var adjusedParagraphCount = mathHelpers.adjusedValue(item.paragraphs, this._adjustParagraphCount);
    ...
    var scores = [
      ...
      adjusedStyleRatio, adjusedSentenseCount, adjusedParagraphCount
    ];
    ...
    commonParent.push({
      ...
      'discreteSentense': item.sentenses,
      'sentenses': adjusedSentenseCount,
      'paragraphs': adjusedParagraphCount,
      'match': adjusedMatch,
      'attr': item.node.attr,
      'node': item.node,
      'text': item.node.getText(),
