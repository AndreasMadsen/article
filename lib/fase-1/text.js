...
TextAlgoritme.prototype.end = function () {
  // Take the 25 best elements mesured by the density
  ...
  for (var i = 0; i < l; i++) {
    for (var j = 0; j < l; j++) {
      ...
      if (commonParentMap.hasOwnProperty(parentNode.position) === false) {
        ...
        var linearray = parent.text.split(LINE_SPLIT);
        var shortlines = 0;
        ...
        var textarray = parent.text.split(WORD_SPLIT);
        for (var c = 0; c < r; c++) {
  ...
  for (var n = 0, t = keys.length; n < t; n++) {
    ...
    likelihood = mathHelpers.distance([adjusedCount, adjusedDensity, adjusedShortline, adjusedMatch]);
    minimum = Math.min(adjusedCount, adjusedDensity, adjusedShortline, adjusedMatch);
