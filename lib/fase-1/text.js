...
TextAlgoritme.prototype.end = function () {
  ...
  for (var n = 0, t = keys.length; n < t; n++) {
    ...
    if (this._metaAppear.length > 0) {
      ...
      likelihood = mathHelpers.distance([adjusedCount, adjusedDensity, adjusedMatch]);
      minimum = Math.min(adjusedCount, adjusedDensity, adjusedMatch);
    } else {
      console.log('relevant');
      likelihood = mathHelpers.distance([adjusedCount, adjusedDensity]);
