...
TextAlgoritme.prototype.end = function () {
  ...
  for (var n = 0, t = keys.length; n < t; n++) {
    ...
    likelihood = mathHelpers.distance([adjusedCount, adjusedDensity, adjusedShortline, adjusedMatch, adjusedLinebreak, adjusedClassscore]);
    minimum = Math.min(adjusedCount, adjusedDensity, adjusedShortline, adjusedMatch, adjusedLinebreak, adjusedClassscore);
...
TextAlgoritme.prototype.result = function () {
  ...
  return this._textContainer.length === 0 ? null : this._textContainer[0].text;
};
