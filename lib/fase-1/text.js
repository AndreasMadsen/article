...
function linebreakScore(x) {
  if (x > 5) return 1;
  else return 0.477 * Math.sqrt(x) - 0.0106 * Math.pow(x, 2) + 0.2;
}
...
TextAlgoritme.prototype.end = function () {
  ...
  for (var n = 0, t = keys.length; n < t; n++) {
    ...
    likelihood = mathHelpers.distance([adjusedCount, adjusedDensity, adjusedShortline, adjusedMatch, adjusedLinebreak]);
    minimum = Math.min(adjusedCount, adjusedDensity, adjusedShortline, adjusedMatch, adjusedLinebreak);
